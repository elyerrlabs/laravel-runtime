...
class ComponentMakeCommand extends GeneratorCommand
{
    ...
    protected function buildClass($name)
    {
        ...
        return str_replace(
            ['DummyView', '{{ view }}'],
            'view(\'' . $this->prefixModule() . '::'  . $this->getView() . '\')',
            parent::buildClass($name)
        );
    }
    ...
    private function decodeComposer()
    {
        ...
        if (!file_exists($path)) {
            throw new ReportError(__('composer.json file not found'), 404);
        }
    ...
    private function prefixModule()
    {
        $moduleName = strtolower(explode('/', $this->decodeComposer()->name)[1]);
